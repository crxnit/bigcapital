...
# ~1.5 GB on disk after extraction into overlayfs). On a busy day with 5+
# deploys, the VPS' /var/lib/containerd hits "no space left on device"
# mid-extract.
#
# Previously this used `--filter "until=72h"` to keep 3 days of rollback
# images, but that meant nothing built today got pruned and the disk filled
# up regardless. Switched to unfiltered `-af`: prunes ALL images not used
# by a *running* container. The currently-deployed image stays (its
# container is running); the previous deploy's image gets reclaimed on the
# NEXT deploy after this one. Rollback is via `docker compose pull` of an
# older sha tag from GHCR, not via locally-cached old images.
