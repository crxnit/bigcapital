...
- **Bill direct-account allocations (no items required)** — A vendor bill can carry direct expense-account allocations alongside (or instead of) itemized line entries. New `bill_expense_categories` table (`id, bill_id, expense_account_id, index, description, amount`); Bill model has a `categories` HasMany relation and `total` getter now adds `categoriesTotal` to the items subtotal. GL emission adds one DR per category against the chosen expense account; AP CR sums items + categories. Submit allows `entries` to be empty as long as categories is non-empty (and vice versa); both can be set. Service-side cascade in `DeleteBill` removes child rows. Webapp Bill form renders a "Direct expense allocations" table below the items table — row shape `Account + Description + Amount`. **Tenant migrations must run on deploy** — after upgrading the server image run `docker compose up -d --force-recreate database_migration`. Symmetric Invoice feature (revenue-account allocations) is tracked as the next follow-up; v1 ship is bills-only. Percent-typed amounts, landed-cost flag, CSV import for categories, and PDF/CSV export inclusion are intentional v1 scope limits.
- **Vendor credit direct-account allocations (no items required)** — Mirror of the bills feature for the vendor credit (vendor → customer credit) form. New `vendor_credit_expense_categories` table (`id, vendor_credit_id, expense_account_id, index, description, amount`); VendorCredit model has a `categories` HasMany relation and `total` getter now adds `categoriesTotal`. GL emission adds one CR per category against the chosen expense account (vendor credit reverses the bill, so opposite side from bills); AP DR sums items + categories via `totalLocal`. Submit allows `entries` to be empty as long as categories is non-empty (and vice versa). Service-side cascade in `DeleteVendorCredit` removes child rows. Webapp Vendor Credit form renders the same "Direct expense allocations" table below the items table. **Tenant migrations must run on deploy** — `docker compose up -d --force-recreate database_migration`. Same v1 scope limits as bills (no percent amounts, no PDF/CSV inclusion).
...
- **`FSelect` dropdowns (and `AccountsListFieldCell`) filter in-memory** — the form provider must request `page_size: 10000` (or similar large value) on `useVendors` / `useCustomers` / `useAccounts` / `useItems` / `useBranches` / `useProjects` / `useWarehouses` / `useTaxRates` and every other list hook feeding a select control. The symptom isn't just an unsearchable dropdown: an _already-selected_ id whose record lives past page 1 renders as an **empty dropdown row** (no label found in the in-memory list), which on edit-form re-open looks like "the value got deleted." Bit `useVendors` / `useCustomers` originally (`packages/webapp/src/containers/Expenses/ExpenseForm/ExpenseFormPageProvider.tsx`), then `useAccounts` in the same provider for the payment-splits and categories pickers (commit `3b6d4b105`). When adding or auditing a form provider, grep every `useXxx(` call for an empty or missing `page_size`.
...
- **`Scope.TRANSIENT` + property injection + class-field arrow methods is a Nest trap.** A `@Injectable({ scope: Scope.TRANSIENT })` service whose API surface is defined via class-field arrow assignments (`public foo = async (...) => {...}`) and whose consumers wire it via `@Inject(Cls) prop: Cls` will throw `<prop>.<method> is not a function` at runtime — even though the constructor sets the field, Nest's per-context proxy wrapper for TRANSIENT-scoped injections exposes a value that lacks the instance methods. Bit `CashBasisProjection` until fixed in `9fa81cd14`. **Default rule for new services:** prefer **constructor injection** (`constructor(@Inject(X) private x: X) {}`) over property injection, and use the **default singleton scope** unless you truly need TRANSIENT (e.g. per-request mutable state). The tenant model proxies are CLS-aware factories, so singleton consumers still resolve per-tenant correctly. If TRANSIENT is genuinely needed, define the methods as regular `public foo(...)` declarations rather than class-field arrows so they live on the prototype.
