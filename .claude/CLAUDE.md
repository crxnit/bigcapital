...
- **Bidirectional cashflow categorization for refunds** — `OtherIncome` ↔ `OtherExpense` work in either direction (deposit-as-OtherExpense = vendor refund reducing expense; withdrawal-as-OtherIncome = customer refund reducing income). Owner/Transfer pairs already model both directions, so they keep direction match. Sub-form labels direction-aware via `autofillCategorizeValues.isDepositTransaction`. **GL writer recovers direction from `cashflow_transactions.uncategorized_transaction_id` source-row sign** — categorize flow populates the column, `BankTransactionGL` reads `source.amount > 0` for OtherIncome/OtherExpense and overrides the fixed `typeMeta.direction`. The original feature commit (`68a420fc4`) wired validator + UI but left the GL reading typeMeta verbatim → refunds posted the wrong-direction journal. Fixed 2026-05-28 in commit `3562a0abb`; old rows need a back-link backfill + per-row Edit Category re-save (recipe in commit message).
- **Credit card as Bill payment account** — `New Payment Made` AccountsSelect filter and `BillPaymentValidators.getPaymentAccountOrThrowError` both allow `CREDIT_CARD` alongside `BANK` / `CASH` / `OTHER_CURRENT_ASSET`. GL writer is account-agnostic (always credits `paymentAccountId`), so a CC payment posts `DR AP / CR Credit Card Liability` — the AP-to-CC liability transfer that mirrors "pay vendor with credit card." Commit `5525a688a`.
- **Vendor-credit refunded-amount sync** — `RefundSyncVendorCreditBalanceSubscriber` was wholly commented out in the NestJS migration, so creating a `RefundVendorCredit` never bumped the parent's `refundedAmount` → status stuck on Open even when fully refunded. Re-enabled and routed through `RefundSyncCreditRefundedAmount` (per-id `.findById(vendorCreditId).increment(...)` — there's a sibling table-wide service in the same dir with no `where('id', …)`; it's a footgun, leave it uncalled). Commit `d40e5b275`. Backfill SQL: `UPDATE VENDOR_CREDITS vc LEFT JOIN (SELECT VENDOR_CREDIT_ID, COALESCE(SUM(AMOUNT),0) AS refunded FROM REFUND_VENDOR_CREDIT_TRANSACTIONS GROUP BY 1) r ON r.VENDOR_CREDIT_ID = vc.ID SET vc.REFUNDED_AMOUNT = COALESCE(r.refunded, 0);`.
...
- **Vendor credit direct-account allocations** — Mirror of bills (opposite GL direction: CR per category). `vendor_credit_expense_categories`; same patterns. **`total` getter MUST be `subtotal - discountAmount + adjustment` (matches Bill / SaleInvoice / CreditNote)** — `subtotal === amount` already includes `categoriesTotal` via the DTO transformer; adding it again doubles the AP debit (regression caught + fixed 2026-05-28, commit `771403d4d`).
...
- **Descriptive report-PDF download filenames** — All 17 financial-statement PDF dialogs (`BalanceSheet`, `ProfitLossSheet`, `CashFlowStatement`, `GeneralLedger`, `Journal`, `TrialBalanceSheet`, `ARAgingSummary`/`APAgingSummary`, `CustomersTransactions`/`VendorsTransactions`, `CustomersBalanceSummary`/`VendorsBalanceSummary`, `SalesByItems`/`PurchasesByItems`, `InventoryItemDetails`/`InventoryValuation`, `SalesTaxLiabilitySummary`) build the download filename via shared `buildReportPdfFilename(reportSlug, httpQuery)` in `packages/webapp/src/containers/FinancialStatements/common.tsx`. Output `report-slug_YYYY-MM-DD_to_YYYY-MM-DD_basis_YYYY-MM-DD-HHmm.pdf` (range) or `report-slug_as-of_YYYY-MM-DD_..._YYYY-MM-DD-HHmm.pdf` (as-of). Handles `from_date`/`to_date`, `as_date`, `basis`/`accounting_method`. Replaces hardcoded `download={'invoice.pdf'}` (copy-paste from invoice dialogs) on Balance Sheet, Cash Flow, Vendor Transactions, Vendor Balance Summary, Purchases by Items. Commit `a2fbb5446`.
- **Bank/cash account cards (cashflow-accounts) overhaul** — Cards now (1) default to alphabetical (name A–Z) with a server-side **sort dropdown** in `CashFlowAccountsActionsBar`, (2) show a colored circular **type badge** top-right (bank=blue / cash=green / credit-card=violet; clearing=teal `swap-horiz`), and (3) group into titled **sections**: Cash → Checking → Savings → Bank → Clearing → Credit Card (empty hidden; unmatched → "Other Accounts" so nothing is dropped). Checking/Savings/Clearing come from a new nullable `accounts.bank_account_subtype` column (migration `20260530160000`; values `checking|savings|clearing|other|null`) set via an **"Account Subtype"** dropdown in the Account dialog — shown for **Bank + Other-Current-Asset** types (Checking/Savings bank-only; Clearing both). `GetBankAccounts` returns cash/bank/credit-card **plus** any account tagged `bank_account_subtype='clearing'`, so Other-Current-Asset clearing accounts (Square/Stripe/DoorDash settlement) surface on the cashflow page **without changing their balance-sheet type**. New column flows through the DTO spread + transformer `toJSON()` (no transformer change). Commits `a6e9f0070`, `7e1ac92be`, `3ee410e72`, `6640838e8`. **Run tenant migrations.**
- **Invoice & Bill list default sort + per-user persistence** — both lists default **oldest-first** by date; the chosen sort persists across full reload via shared `store/persistListSort.ts` (`createListSortPersistConfig` — persists only `tableState.sortBy` via a redux-persist transform + `autoMergeLevel2`). See the "List table-state defaults" gotcha. Commit `0771c21e7`.
- **"Bill(s)" → "Purchase Invoice(s)" relabel** — all user-facing labels standardized via `lang/en/index.json` **values only**; i18n keys, routes (`/bills`), models, API, and `resource:'Bill'`/`value:'Bill'` code identifiers unchanged. "Billing" / "Billed to/from" / "Billable" / "Bill To" (customer) deliberately left as-is. Commit `6858851b2`.
- **Removed global Lemon Squeezy script** — `app.lemonsqueezy.com/js/lemon.js` no longer loaded in `index.html` (was on every page incl. pre-auth login — referer leak + supply-chain surface). The three subscription call sites guard `window.LemonSqueezy?.` with a `window.open(url)` fallback. Self-hosted instance doesn't sell SaaS seats. Commit `37a007768`.
...
- **NestJS-migration commented-out subscribers compile fine and register nothing.** `RefundSyncVendorCreditBalanceSubscriber.ts` was a whole-file `//`-block-commented Typedi subscriber the migration left in place; TS compiles, Nest sees no provider, the domain effect (vendor credit balance sync) silently never runs until a use case trips it. When opening a subscriber file with the `// @Service()` / `// @Inject()` shape at the top, treat it as broken — port to `@OnEvent` + `@Injectable` + module registration, don't just delete. Same shape may lurk in other migration-era files; `grep -l '// @Service()\|// @Injectable' packages/server/src/modules/**/subscribers/*.ts` flags candidates.
- **List-query DTOs validating `sortOrder` with `@IsEnum(ISortOrder)` 400 on the webapp's lowercase `asc`/`desc`.** `transformTableStateToQuery` always emits lowercase `sort_order`, but `ISortOrder` is uppercase `ASC`/`DESC` — so the first time a list sends a sort to such an endpoint it fails (symptom: list shows "no … with current filter criteria", Network response `message[].constraints.isEnum`). Sibling list DTOs extending `DynamicFilterQueryDto` use a plain `@IsString()` and are immune. Fix = `@Transform(({value}) => typeof value === 'string' ? value.toUpperCase() : value)` **before** `@IsEnum(ISortOrder)` (normalize, keep strict validation; downstream Knex `orderBy` is case-insensitive). Bit `BankAccountsQueryDto` + `GetAccountsQueryDto` when cashflow-accounts got a default name-sort; commit `ec13d5b99`. **When adding a default/served `sortBy` to a list, grep its server DTO for `@IsEnum(ISortOrder)` first.**
...
- **Trial-balance audit recipe**: `SELECT REFERENCE_TYPE, REFERENCE_ID, SUM(DEBIT)-SUM(CREDIT) AS diff FROM ACCOUNTS_TRANSACTIONS GROUP BY 1, 2 HAVING ABS(SUM(DEBIT)-SUM(CREDIT)) > 0.005` names every unbalanced (transaction, leg-set) in one query. Once it names a doc, **first step is server logs at the surviving GL row's `CREATED_AT`** — `docker compose logs --since … --until … server | grep -A 20 'ERROR \[Event\]'`. Nest's event-emitter wraps subscribers in try/catch and only logs (`EventSubscribersLoader.wrapFunctionInTryCatchBlocks` in the stack), so per-row INSERT failures (FK violation, `ER_DATA_TOO_LONG`, etc.) bail the saveEntries `for…await` loop after the AR/AP debit lands, UoW commits, green toast. The swallowed-INSERT class is more common than concurrency — don't chase Promise.all / async.queue hypotheses without the log check first. Join `ITEMS_ENTRIES` (or relevant child) on `(REFERENCE_ID, ITEM_ID, ACCOUNT_ID)` for missing legs once the failure mode is identified.
- **`ACCOUNTS_TRANSACTIONS.NOTE` is `TEXT`** (migrated from `varchar(255)` on 2026-05-28, commit `227b77893`). The GL writers copy `ItemEntry.description` verbatim into the ledger `note`; descriptions over 255 chars used to throw `ER_DATA_TOO_LONG` and lose every credit leg downstream of the AR debit. Surfaced as the SaleInvoice 114 trial-balance break.
...
- **Three things are coupled — pagination order, running-balance walk, and infinite-scroll direction — change them together.** The register reads oldest→newest (top→bottom). `GetBankAccountTransactionsRepo` paginates `date asc, created_at asc`; the running balance is walked oldest→newest (opening balance = net of the `pageSize × (page-1)` older rows, zero on page 1; apply-this-row-then-capture so each row shows the balance _after_ it); the client `AccountTransactionsDataTable` sets `initialSortBy={[{ id:'date', desc:false }]}` to MATCH the server so the initial client sort is a no-op and infinite-scroll pages append cleanly at the bottom. A client sort that disagrees with the server page order re-sorts each newly loaded page to the top → "list won't load, scroll+click a header to fix." Commit `77be587aa`. Validate any running-balance change with a window-function cumulative sum: `SUM(DEBIT-CREDIT) OVER (ORDER BY DATE, CREATED_AT)` — oldest row = its own amount, newest row = account total.
- **Possible open issue**: react-virtualized `WindowScroller` in shared `components/Datatable/TableVirtualizedRows.tsx` may need an initial scroll to render rows on first mount; not yet fixed (would touch a shared component — verify on UAT before changing).
...
- **Currency-math gates need ε tolerance, not `> 0`.** Floating-point dust on sums like `315.72 - (183.42 + 0 + 87.38 + 44.92)` is ~1e-13: formats as `$0.00` but `> 0` is true. Symptom: payment forms' excess-credit dialog fires on a perfectly balanced payment, asking to record `$0.00 as credit`. Pattern: every `if (currencyDiff > 0)` in payment / allocation / reconcile flows is suspect; use `> 0.005` (half a cent, matches split-payment validator tolerance). Commit `a5893e40c` covers `PaymentMadeForm` + `PaymentReceiveForm`; grep other money gates when touching this area.
- **New report PDF dialogs MUST use `buildReportPdfFilename`** from `FinancialStatements/common.tsx` for the `<AnchorButton download={...}>` prop. The original `invoice.pdf` bug on 5 reports was copy-paste from invoice dialogs; the helper is the deterrent. Sanity check: `grep -rn "download={'" packages/webapp/src/containers/FinancialStatements/` should print nothing.
- **Entry-total helpers must coerce `amount` to a number — never `sumBy(rows, 'amount')`.** The always-present trailing empty line carries `amount: ''`; lodash `sumBy` then concatenates (`2500 + '' === "2500"`), and any downstream arithmetic (e.g. `itemsTotal + categoriesTotal` in `useInvoiceSubtotal`) turns `"2500" + 0` into `"25000"` — a silent 10× on whole-dollar documents (decimals like `334.59` hide it). `getEntriesTotal` in `Entries/utils.tsx` sums `toSafeNumber(entry.amount)`; keep it that way and apply the same coercion to any new sum over form rows. Commit `0e40703a3`.
- **Document discounts: a `null`/missing `discount_type` means fixed amount, NOT percentage.** Webapp edit forms strip null fields on hydrate (`transformToForm`), so an untyped discount round-trips without `discount_type`; default it to `'amount'` on hydrate so the unit dropdown and persisted value are concrete. Server side, the model `discountAmount` getter (all six doc models) must read `=== DiscountType.Percentage ? subtotal*discount/100 : discount` — treating anything-not-Amount as percentage made a `$111.53` fixed discount post as `111.53%` → `$497.56`. Commit `193e615d2`. See `FORK-BUG-HISTORY.md`.
- **List table-state defaults + persistence (invoices/bills pattern).** A landing list's default sort lives in `defaultTableQuery.sortBy` in its `store/<Resource>/*.reducer.tsx` (e.g. `[{ id: 'invoice_date', desc: false }]` / `[{ id: 'bill_date', desc: false }]` — both default **oldest-first**), mapped to `column_sort_by`/`sort_order` by `transformTableStateToQuery` — the same server path a header click uses. Three things make it stick:
  - **Across an Edit round-trip (popup → `/…/edit` → back):** do NOT call `reset…TableState()` on list unmount, and pass `initialSortBy={tableState.sortBy || []}` + `autoResetSortBy={false}` to the `DataTable` so the header reflects state and a sort survives data reloads. Redux survives SPA nav.
  - **Across a full page reload:** the reducer's redux-persist CONFIG must persist the sort. Use the shared `createListSortPersistConfig(STORAGE_KEY)` from `store/persistListSort.ts` (NOT `whitelist: []`). It persists ONLY `tableState.sortBy` (via a `createTransform` + `autoMergeLevel2` reconciler) so the chosen sort is restored on reload while page index and filters fall back to defaults. Pass the same CONFIG object to both `persistReducer` and `purgeStoredState`.
  - **The server must accept the sort key.** `column_sort_by` is resolved by `DynamicListService` via `model.getField(key)`, so the `sortBy[0].id` must be a field key in the model's `*.meta.ts` `fields` (e.g. `invoice_date`, `bill_date`; for cashflow accounts `name`/`code`/`balance`/`type`/`created_at` — note `balance`→`amount` column, not `amount`). An unknown key throws server-side.
  - Initial pattern: commit `e569bfc13` (invoices, sort across SPA nav). Extended 2026-05-30: oldest-first defaults + reload persistence + bills parity (commits `0771c21e7`, and `a6e9f0070` for the cashflow-accounts sort dropdown).
- **Form `onError` handlers read `response.data.errors`, but the global class-validator pipe returns `response.data.message` (an array of ValidationError objects).** The server pipe (`common/pipes/ClassValidation.pipe.ts`) is `whitelist: true` (strips undecorated props, does NOT `forbidNonWhitelisted`), so a 400 on save is a field-level DTO validation failure whose detail lives in `message`, not `errors`. Result: the UI shows a silent `AxiosError` with no toast. When debugging a save 400, read the Network response `message` array (it names the field + constraint); when touching a form's `onError`, surface `message` too so validation failures aren't invisible.
- **Blueprint `<Tooltip>`/`<Popover>` with a dynamic child must guard against empty values.** `<Tooltip>` wraps a `<Popover>`; a `null`/`''`/`undefined` child renders no target and logs `[Blueprint] <Popover> requires target prop or at least one child element.` (harmless console noise, but fires on every blank cell). The shared `TextOverviewTooltipCell` (`components/DataTableCells/`, reused across the Invoice/Bill/Estimate/Expense/CreditNote/ManualJournal detail drawers) returns `null` early when its value is empty — keep that guard, and apply the same pattern to any new Popover/Tooltip whose single child is a data value (e.g. a nullable line-item description). Diagnostic lesson from the same fix: this warning fires on the **load→render** path (`onSuccess → setData → re-render`), NOT on a request failure — get the Network-tab status + body before theorizing about a 400. Commit `48ba12e8f`.
- **Account `account_type` is locked on edit (by design).** `AccountDialog` disables the type select in Edit / NewChild / NewDefinedType modes (`getDisabledFormFields`). Changing a posted account's type is risky (balance-sheet classification, report grouping), so there's deliberately no UI to convert e.g. Other-Current-Asset → Bank. When a feature needs an account in a different "bucket", prefer a non-type marker over a type change / SQL conversion — the `bank_account_subtype='clearing'` tag is the precedent: surface the account by tag (and widen the list query to include the tag), keep its real type. Commit `6640838e8`.
