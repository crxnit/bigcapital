...
- **Env-presence assertions / fail-fasts belong in `main.ts bootstrap()`, NOT in a `registerAs()` config factory.** A config factory runs in EVERY process that builds `ConfigModule` — including the `database_migration` service (`cli.js system/tenants:migrate`), whose env carries only DB vars. A `throw` in the factory (e.g. asserting `JWT_SECRET` is set) kills the migration container (exit 1) → deploy aborts, even though migrations don't need the secret. Keep config factories side-effect-free; assert presence in `bootstrap()`, which only runs for the API server. Bit the JWT-secret fail-fast (commits `3d16e33e3`→`8c47d64ba`); see [[project_jwt_secret_env_mismatch]].
...
- **A `@OnEvent` subscriber (or any `@Injectable`) NOT listed in its module's `providers` is never instantiated — Nest registers no listener, so the effect silently never runs.** Same blast radius as the commented-out trap above, but the class looks complete. Bit the Branches/Warehouses `activate` subscribers (only 3 of 10 branch + 5 of 7 warehouse were in `providers`) → multi-branch/-warehouse activation stamped the primary id onto only some transaction types. When adding a `subscribers/Activate/*` (or any event handler), add BOTH it and the integration class it injects to the module `providers`. Commit `ce98a0dea`.
- **A tenant model-proxy ctor param MUST carry `@Inject(Model.name)`.** The proxy is provided under a string token (`Model.name`), so a bare `private readonly fooModel: TenantModelProxy<typeof Foo>` has no resolvable token → Nest fails to construct the provider at boot (`Nest can't resolve dependencies … argument at index [N]`). Several never-registered helper classes had this latent (e.g. `SaleInvoiceActivateBranches`), so it only surfaced when they were finally added to `providers`. Grep test when wiring a new model-injecting class: every `TenantModelProxy<typeof X>` ctor param has a matching `@Inject(X.name)` directly above it. Commit `ce98a0dea`.
...
- **Mail transport must OMIT the `auth` key when no SMTP username is set.** `Mail.module.ts` builds the nodemailer transport; passing `auth: { user, pass }` with empty/undefined creds makes nodemailer still attempt PLAIN and fail with `Missing credentials for "PLAIN"` — so IP-authorized relays (Google Workspace SMTP relay `smtp-relay.gmail.com:587`, Postfix `permit_mynetworks`, SES VPC endpoints) can't send. Attach `auth` only when `mail.username` is truthy: `...(username ? { auth: { user, pass } } : {})`. A non-empty username re-enables auth. Commit `e0e1eb708`; live-verified against the Sofi relay (no-auth → 250 OK). Sofi's `.env` sets `MAIL_USERNAME=`/`MAIL_PASSWORD=` empty by design.
...
- **`TENANT_DB_NAME_PERFIX` (sic — upstream typo) must match between the create and connect paths, or org-build silently hangs.** The tenant-DB CREATE path (`TenantDBManager.getDatabaseName`) honors `tenantDatabase.dbNamePrefix` (env `TENANT_DB_NAME_PERFIX`, default `bigcapital_tenant_`), but the tenant CONNECTION factory (`Tenancy/TenancyDB/TenancyDB.module.ts`) used to **hardcode** `bigcapital_tenant_${organizationId}`. So a custom-prefixed instance CREATED `<prefix>_<org>` but CONNECTED to `bigcapital_tenant_<org>` → `ER_BAD_DB_ERROR: Unknown database` on migrate → the BullMQ org-build job fails + retries, tenant stuck `building`, the signup wizard spins forever at "Initializing" (no UI error). Symptom is server-log-only; the form/Network look clean. Fixed to read the config prefix (commit this session). Sofi was the first instance to set a custom prefix and exposed it; every prior instance used the default (`bigcapital_tenant_35i5f1mo1phc5w`), so it never bit. Rule: new tenant-DB-name code reads `configService.get('tenantDatabase.dbNamePrefix')` — never a literal `bigcapital_tenant_`. Dedicated boxes gain nothing from a custom prefix; keep the default.
...
- **Deploy stack**: `docker/sofis-bc/` (sibling of `docker/sandbox-bc/`). FQDN **`books.sofisminidonuts.com`** confirmed (A record → the VPS). Client logs in directly — **no JJOC OAuth edge** (Bigcapital JWT only + signup lockdown); `security-headers`+`rate-limit` only. Offsite restic backups mandatory (no sibling fallback). Runbook: `docker/sofis-bc/README.md`. Uses **host Traefik** (bundled-Traefik question resolved 2026-06-13/14 → dropped, joins `portal-net`, commit `079b17fe7`). Pinned `sha-e0e1eb7` (manual updates).
  - **LIVE + GO-LIVE COMPLETE 2026-06-14**: stack serving (`/api/health` ok, valid LE cert), mail working (Google Workspace SMTP relay, no-auth), backups live + restore-drilled. **Org created in-browser + signup locked** (`GET /api/auth/meta` → `{"signup_disabled":true}`; owner `john@jjocllc.com`); empty COA = QB import target. **DB naming is DEFAULT** (system `bigcapital_sofis_system`, tenant `bigcapital_tenant_<org>` — the custom tenant prefix tripped the §"Knex/MariaDB" `TENANT_DB_NAME_PERFIX` hardcode bug, fix `6a568b577`; dedicated boxes keep the default). Remaining = QB migration only (below).
...
- **Never `source` a docker `env_file` as a bash script.** `env_file` is literal `KEY=VALUE` (whole RHS is the value, no shell quoting), so a legit value like `MAIL_FROM_NAME=Sofi's Mini Donuts Books` (apostrophe — also spaces, `#`) aborts `source` with `unexpected EOF while looking for matching '` → the script dies before doing anything. Backup scripts that need the stack's DB/S3 creds must parse it literally: `while IFS='=' read -r k v; do [[ "$k" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] && export "$k=$v"; done < <(grep -vE '^[[:space:]]*(#|$)' "$ENV_FILE")`. Only the restic env (a real `export VAR=...` shell file) may be `source`d. Bit `backup.sh` + `deploy/vps-backup.sh` (silent — nightly backup never ran); fixed commit `43effa5d6`, post-mortem in `docs/FORK-BUG-HISTORY.md`.
...
**Extended 2026-06-14** (trx-threading + auth-hardening sweep, commits `2f5164036`→`8c47d64ba`): threaded `trx` into UoW-escaping writes in CreateAccount / EditCustomer / EditVendor / Edit*OpeningBalance* (account/contact create+edit), the publish patch+re-fetch in PublishExpense / PublishInventoryAdjustment / PublishManualJournal, and CreateBranch / EditBranch / EditWarehouse; serialized the `recordInventoryTransactions` per-row writes (were `Promise.all` on a shared trx — the documented parallel-trx trap) and passed `trx` in `SyncItemTaxRateOnEditTaxRate`. Closed the **hardcoded-`123123` JWT fallback** auth bypass (reads `APP_JWT_SECRET || JWT_SECRET`, fail-fast in `main.ts bootstrap()` — see [[project_jwt_secret_env_mismatch]]) and added **RBAC guards** to SaleReceipts / TransactionsLocking / Organization-update / Roles / Users / UsersInvite (within-tenant privesc — see [[reference_rbac_permission_model]]). **Fixed 2026-06-15** (commit `ce98a0dea`): CreateBranch/EditBranch now emit real `events.branch.onCreate/onEdit` (added the missing event keys + populated payload interfaces) instead of borrowed `warehouse.onEdit`; registered the 7 missing branch + 2 missing warehouse `activate` subscribers (and their integration providers) so multi-branch/multi-warehouse activation stamps the primary id onto ALL transaction types, not just the few that were wired — plus the prereq fixes below.
