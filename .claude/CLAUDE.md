...
- **Package manager**: `pnpm`. **Never** `pnpx` / `pnpm dlx` in committed hooks/scripts (ignores lockfile). Use `pnpm exec <bin>`.
...
Index only — read linked code / commit / `FORK-BUG-HISTORY.md` for detail.
...
- **Staging `.env` Square vars** (`WEBAPP_BASE_URL`, `SQUARE_APPLICATION_ID`, `SQUARE_APPLICATION_SECRET`, `SQUARE_APPLICATION_ACCESS_TOKEN`, `SQUARE_ENVIRONMENT`, `SQUARE_OAUTH_REDIRECT_URL`, `SQUARE_TOKEN_ENCRYPTION_KEY`) — template at **`deploy/staging-bc.env.example`**. **Staging targets Square `production`** (real account) → needs its OWN production OAuth app (one redirect URI/app, distinct from sandbox.bc). Callback: `https://staging.bc.jjocllc.com/api/integrations/square/oauth/callback`. Compose passes all 8 via `${VAR}` `environment:` block → edit `.env` + `docker compose -f docker-compose.ghcr.yml up -d --force-recreate server`. Token key = 64 hex (`openssl rand -hex 32`), stable once tokens stored. **PENDING on VPS**: paste the 3 production secrets (App ID/Secret/PAT) into `/srv/portal/clients/staging-bc/.env` + recreate. Webhook delivery also needs a Traefik router for `/api/integrations/square/webhooks` (priority ≥ 20, no OAuth middleware).
- **Trivy gate is BLOCKING** (`exit-code: '1'`, flipped 2026-05-31). Webapp image clean; all findings server-image. devDep-leak cleanup DONE 2026-05-31 (155→113 HIGH/CRITICAL, 7→3 CRITICAL, image 1.88GB→1.19GB): `packages/server/Dockerfile` strips root `prepare: husky install` then clean `pnpm install --prod` (old `pnpm add/remove husky` dance re-hydrated all devDeps); build/test tooling moved `dependencies`→`devDependencies` in `shared/email-components`, `shared/pdf-templates`, root `package.json`. Lockfile edited by hand (only `importers:` dev/prod categorization; resolved set byte-identical — avoid `pnpm install` regen which reserializes ~32k cosmetic lines). **Gotcha:** clean `--prod` exposes runtime-imported devDeps — `mustache` (used by `Mail.ts`) was a devDep → migration container `Cannot find module 'mustache'`. Moved to `dependencies`, commit `699c96c7e`. Validate dep pruning by load-testing the prod image: `docker run --entrypoint node <img> packages/server/dist/cli.js --help` → 0 `MODULE_NOT_FOUND` (`@nestjs/testing` is spec-only, excluded). **Gate flip 2026-05-31**: scoped pnpm `overrides` (root `package.json`, `pkg@major` selectors) bumped 2 same-major CRITICALs + 3 safe HIGHs — `form-data`→4.0.4, `fast-xml-parser`→4.5.5, `cross-spawn`→7.0.6, `fast-uri`→3.1.2, `validator`→13.15.22 — findings 155→105, CRITICAL 7→**1** (only `@casl` left, needs v6 major). `.trivyignore` is now a **dated 46-CVE baseline** (expiry 2026-08-31) of cross-major transitive debt that can't be safely bumped (`axios`×5 via firebase-admin/plaid, `multer` 1→2, `tar` 6→7, `minimatch`, `path-to-regexp`, `@casl` v6, `xlsx` off-npm, `lodash` no-fix) + build-only `pnpm` + dead `cross-spawn` vendored in `mathjs/examples`. Applying `overrides` reserializes the whole lockfile ~32k cosmetic lines (resolved-set delta = only the 5 swaps + 1 transitive) — accepted, unavoidable for any `pnpm install`. Validate gate locally before pushing: `docker run -v .../.trivyignore:/.trivyignore aquasec/trivy image --severity HIGH,CRITICAL --ignore-unfixed --ignorefile /.trivyignore --exit-code 1 <img>` → must exit 0. Full plan + re-scan recipe in `docs/CI-CD.md`.
...
- **Node 24 actions bump — DONE 2026-05-30.** Swept `.github/workflows/*` to node24 majors: `actions/checkout@v6`, `actions/setup-node@v6`, `actions/upload-artifact@v7`, `actions/cache@v5`, `docker/setup-buildx-action@v4`, `docker/login-action@v4`, `docker/build-push-action@v7`, `pnpm/action-setup@v6`. **NOT bumped:** `docker/metadata-action@<SHA>` (security SHA pin), `aquasecurity/trivy-action`/`rtCamp/action-slack-notify` (Docker container actions), `peter-evans/create-pull-request@v6` (bump when touching `generate-openapi.yml`). `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` is the fallback. **`pnpm/action-setup@v6` fails when it runs AFTER `actions/setup-node@v6`** — install pnpm BEFORE Node (order `typecheck.yml` uses). Deleted upstream-leftover container workflows (`build-deploy-develop-container.yaml`, `build-deploy-container.yml`). Remaining: `deploy.yml`, `e2e.yml`, `typecheck.yml`, `generate-openapi.yml`.
- **Containerd layer-extraction flake on rapid back-to-back deploys (2026-05-21).** `failed to extract layer ... no such file or directory` in overlayfs; VPS-side `docker compose pull` retry succeeds. **Monthly housekeeping cron WIRED 2026-05-31** (`deploy/vps-docker-prune.sh` + `deploy/bigcapital-{sandbox,uat}-docker-prune.cron`, 04:30/04:50 UTC on the 1st). Deliberately **scoped** (not `system prune -af`): only the env's own exited containers (by `container_name` prefix) + bigcapital images >14d not backing a container. `vps-deploy.sh:80` does a global `image prune -af` per deploy — confirmed fine. Re-install commands in `docs/CI-CD.md`.
