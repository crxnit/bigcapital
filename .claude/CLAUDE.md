...
- **Per-bank-account logos (cashflow cards)** — each cash/bank/credit-card/clearing account can carry a logo shown top-left on its card **alongside** the type badge (badge stays). Two mutually-exclusive sources: bundled **library** (`bank_account_logo_slug` → `BANK_LOGO_LIBRARY` in `@/constants/bankLogos.ts`; assets `webapp/public/bank-logos/<slug>.svg` served at `/bank-logos/<slug>.svg`, resolved client-side) OR **custom upload** (`bank_account_logo_key` → Attachments/S3 pipeline like the org logo; resolved to presigned/proxied `bank_account_logo_uri` in `GetBankAccounts` via `GetAttachmentPresignedUrl` — `BankAccountsModule` imports `AttachmentsModule`). Set via "Bank Logo" field in `AccountDialogFormContent` (library `FSelect` + reused `CompanyLogoUpload`; picking one clears the other; transient `_logo_file` uploaded on submit then stripped in `transformFormToReq`). Card src via `resolveBankAccountLogoSrc(slug, uri)` (slug wins). Both columns are scalar passthrough (migration `20260531170000`); `AccountTransformer` excludes only `plaidItem` so the edit form round-trips them (no hydrate-empty wipe). Shipped logos are **generic placeholders** — replace `public/bank-logos/<slug>.svg` in place; **don't rename slugs** (persisted on accounts). Add a bank = drop SVG + one `BANK_LOGO_LIBRARY` line (a dropped file alone won't appear in the picker). Commits `addb12129`, `45e3ad96d`, `9845eeb89`. Run tenant migrations.
