...
- **Staging `.env` Square vars** (`WEBAPP_BASE_URL`, `SQUARE_APPLICATION_ID`, `SQUARE_APPLICATION_SECRET`, `SQUARE_APPLICATION_ACCESS_TOKEN`, `SQUARE_ENVIRONMENT`, `SQUARE_OAUTH_REDIRECT_URL`, `SQUARE_TOKEN_ENCRYPTION_KEY`) — were missing → Square silently broken. **Resolved in repo 2026-05-31** (var names verified against `modules/SquareIntegration`, `this.config.get(...)`): template at **`deploy/staging-bc.env.example`**. **Decision: staging targets Square `production`** (real account), so it needs its OWN production OAuth app (one redirect URI per app, distinct from sandbox.bc). Callback: `https://staging.bc.jjocllc.com/api/integrations/square/oauth/callback` (`/api` global prefix). Compose passes all 8 via `${VAR}` `environment:` block, so editing `.env` + `docker compose -f docker-compose.ghcr.yml up -d --force-recreate server` suffices. Token key = 64 hex chars (`openssl rand -hex 32`), must stay stable once tokens stored. **PENDING on VPS**: paste the 3 production secrets (App ID/Secret/PAT) into `/srv/portal/clients/staging-bc/.env` + recreate. Webhook delivery additionally needs a Traefik router for `/api/integrations/square/webhooks` (priority ≥ 20, no OAuth middleware).
- **Trivy gate is warn-only** (`exit-code: '0'`). Webapp image is clean (0); all findings are server-image. **devDep-leak cleanup DONE 2026-05-31** — server prod image was shipping the whole devDependency tree. Fixed: (1) `packages/server/Dockerfile` no longer does the `pnpm add/remove husky` dance (the trailing `pnpm remove` re-hydrated all devDeps) — now strips the root `prepare: husky install` script then does a clean `pnpm install --prod`; (2) build/test tooling moved from `dependencies`→`devDependencies` in `shared/email-components` (vitest, vite-plugin-dts), `shared/pdf-templates` (webpack+loaders), root `package.json` (tsup→pulled esbuild). **155→113 HIGH/CRITICAL, 7→3 CRITICAL, image 1.88GB→1.19GB.** Lockfile changed by hand-editing ONLY the `importers:` dev/prod categorization (resolved set byte-identical — avoid `pnpm install` regen, which reserializes the whole file ~32k cosmetic lines under any 9.x). **Gotcha the clean `--prod` install exposed:** the old leaky install masked runtime imports of mis-declared **devDependencies** — `mustache` (imported by `Mail.ts`) was a devDep → migration container hit `Cannot find module 'mustache'`, failing the deploy (deploy.sh halted, no outage). Fixed by moving it to `dependencies` (commit `699c96c7e`). When recategorizing/pruning deps, validate by load-testing the prod image (`docker run --entrypoint node <img> packages/server/dist/cli.js --help` → must show 0 `MODULE_NOT_FOUND`); the prior cross-check found mustache was the only runtime-imported devDep (`@nestjs/testing` is spec-only, excluded from build). **Gate-flip DEFERRED** (user choice): residual 113/53-CVEs is genuine prod-dep debt. To flip `exit-code:'1'`: pnpm `overrides` for the 2 same-major CRITICALs (`form-data`→4.0.4, `fast-xml-parser`→4.5.4 — accept the one-time lockfile reserialization), `.trivyignore` the cross-major transitive debt (`axios`×5/`multer`/`tar`/`minimatch`/`@casl` v6-major) with expiry, then flip. Full plan in `docs/CI-CD.md`.
...
- **Node 24 actions bump — DONE 2026-05-30.** Swept all `.github/workflows/*.yml`/`.yaml` to node24-runtime majors: `actions/checkout@v6`, `actions/setup-node@v6`, `actions/upload-artifact@v7`, `actions/cache@v5`, `docker/setup-buildx-action@v4`, `docker/login-action@v4`, `docker/build-push-action@v7`, `pnpm/action-setup@v6`. Runners are GitHub-hosted ubuntu (meet the node24 ≥ runner-v2.327.1 requirement). **Still on older pins (intentional):** `docker/metadata-action@<SHA>` (security SHA pin — leave), `aquasecurity/trivy-action`/`rtCamp/action-slack-notify` (Docker container actions, no node runtime), `peter-evans/create-pull-request@v6` (not verified for node24 — bump when touching `generate-openapi.yml`). Context: Node 20 deprecated 2025-09-19; GitHub force-defaults node24 on 2026-06-02; removes node20 on 2026-09-16. `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` remains the fallback opt-in if a future action lags.
  - **Follow-up fix 2026-05-31:** the bump regressed `e2e.yml` (red on the bump commit, green before). **`pnpm/action-setup@v6` fails when it runs AFTER `actions/setup-node@v6`** — reorder so pnpm-setup comes BEFORE setup-node (the order `typecheck.yml` already uses and stayed green; same `@v6` + `version: 9`). Fixed + verified green. **CI gotcha for new workflows: install pnpm before Node.** Also **deleted both upstream-leftover container workflows** (`build-deploy-develop-container.yaml` push-on-develop + `build-deploy-container.yml` release/dispatch) — they pushed images to `bigcapitalhq/{webapp,server}` Docker Hub (no creds → red X on every develop push; superseded by `deploy.yml`→`ghcr.io/crxnit`). Remaining workflows: `deploy.yml`, `e2e.yml`, `typecheck.yml`, `generate-openapi.yml`.
- **Containerd layer-extraction flake on rapid back-to-back deploys (2026-05-21).** `failed to extract layer ... link ... no such file or directory` in overlayfs; VPS-side `docker compose pull`, retry succeeds. **Monthly housekeeping cron WIRED 2026-05-31** (`deploy/vps-docker-prune.sh` + `deploy/bigcapital-{sandbox,uat}-docker-prune.cron`, 04:30/04:50 UTC on the 1st). Deliberately **scoped** (not daemon-global `system prune -af`): removes only the env's own exited containers (by `container_name` prefix) + bigcapital images >14d not backing a container — safe on the shared daemon. `vps-deploy.sh:80` does a global `image prune -af` per deploy (reclaims images on every deploy) — **confirmed fine: the VPS daemon is effectively dedicated to bigcapital, so daemon-global pruning has no co-tenant blast radius** (the monthly cron is still scoped, but that's belt-and-suspenders, not a hard requirement). **Fully deployed 2026-05-31** — installed on the VPS (both env dirs + `/etc/cron.d`) and a manual run verified clean. (`install` commands in `docs/CI-CD.md` if re-installing.)
