...
- **List table-state defaults + persistence (invoices pattern).** A landing list's default sort lives in `defaultTableQuery.sortBy` in its `store/<Resource>/*.reducer.tsx` (e.g. `[{ id: 'invoice_date', desc: true }]`), mapped to `column_sort_by`/`sort_order` by `transformTableStateToQuery` — the same server path a header click uses. To remember sort/page/filters across an Edit round-trip (popup → `/…/edit` → back), do NOT call `reset…TableState()` on list unmount, and pass `initialSortBy={tableState.sortBy}` to the `DataTable` so the header reflects it. Redux survives SPA nav; a full reload falls back to the default. Commit `e569bfc13`.
