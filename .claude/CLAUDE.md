...
Index only — read linked code / commit / `FORK-BUG-HISTORY.md` for design detail.
...
- **moment.js `MM` vs `mm`**: `MM` = month, `mm` = minutes. `HH:MM` renders month as the minute (old report-footer bug).
...
- **List table-state defaults + persistence (invoices/bills pattern).** Default sort lives in `defaultTableQuery.sortBy` in `store/<Resource>/*.reducer.tsx` (e.g. `[{ id: 'invoice_date', desc: false }]` / `[{ id: 'bill_date', desc: false }]`, both **oldest-first**), mapped to `column_sort_by`/`sort_order` by `transformTableStateToQuery`. Three things make it stick:
  ...
  - **Across full page reload:** reducer's redux-persist CONFIG must persist the sort — use shared `createListSortPersistConfig(STORAGE_KEY)` from `store/persistListSort.ts` (NOT `whitelist: []`); persists only `tableState.sortBy`. Pass same CONFIG to `persistReducer` and `purgeStoredState`.
...
- **Solo-dev — no PR ceremony.** Single-developer fork. Commit + push directly to `develop`; rides sandbox → UAT automatically. To gate a commit, hold off pushing. Don't open a PR / feature branch unless explicitly asked.
...
- **API rate limit**: NestJS throttler. Set `THROTTLE_GLOBAL_LIMIT=600`, `THROTTLE_AUTH_LIMIT=60`, `THROTTLE_GLOBAL_TTL`/`THROTTLE_AUTH_TTL`=`60000` (defaults 100/10 too strict for SPA). Legacy `API_RATE_LIMIT` NOT used. State in Redis — restart Redis to clear lockouts.
...
- **Trivy gate is BLOCKING** (`exit-code: '1'`, flipped 2026-05-31). Webapp image clean; all findings server-image. devDep-leak cleanup DONE 2026-05-31 (155→113 HIGH/CRITICAL, 7→3 CRITICAL, image 1.88GB→1.19GB): `packages/server/Dockerfile` strips root `prepare: husky install` then clean `pnpm install --prod` (the old `pnpm add/remove husky` dance re-hydrated all devDeps); build/test tooling moved `dependencies`→`devDependencies` in `shared/email-components`, `shared/pdf-templates`, root `package.json`. Lockfile edited by hand (only `importers:` dev/prod categorization; resolved set byte-identical — avoid `pnpm install` regen which reserializes ~32k cosmetic lines). **Gotcha:** clean `--prod` exposed runtime-imported devDeps — `mustache` (used by `Mail.ts`) was a devDep → migration container `Cannot find module 'mustache'`, deploy halted (no outage). Moved to `dependencies`, commit `699c96c7e`. Validate dep pruning by load-testing the prod image: `docker run --entrypoint node <img> packages/server/dist/cli.js --help` → 0 `MODULE_NOT_FOUND` (`@nestjs/testing` is spec-only, excluded). **Gate FLIPPED 2026-05-31** (`exit-code:'1'`): scoped pnpm `overrides` (in root `package.json`, `pkg@major` selectors) bumped the 2 same-major CRITICALs + 3 safe HIGHs — `form-data`→4.0.4, `fast-xml-parser`→4.5.5, `cross-spawn`→7.0.6, `fast-uri`→3.1.2, `validator`→13.15.22 — taking findings 155→105, CRITICAL 7→**1** (only `@casl` left, needs v6 major). `.trivyignore` is now a **dated 46-CVE baseline** (expiry 2026-08-31) of cross-major transitive debt that can't be safely bumped (`axios`×5 via firebase-admin/plaid, `multer` 1→2, `tar` 6→7, `minimatch`, `path-to-regexp`, `@casl` v6, `xlsx` off-npm, `lodash` no-fix) + build-only `pnpm` (strip from runtime in follow-up) + a dead `cross-spawn` vendored in `mathjs/examples`. **Applying `overrides` reserializes the whole lockfile ~32k cosmetic lines (resolved-set delta = only the 5 swaps + 1 transitive) — accepted, unavoidable for any `pnpm install`.** Validate the gate locally before pushing: `docker run -v .../.trivyignore:/.trivyignore aquasec/trivy image --severity HIGH,CRITICAL --ignore-unfixed --ignorefile /.trivyignore --exit-code 1 <img>` → must exit 0. Full plan + re-scan recipe in `docs/CI-CD.md`.
...
- **Node 24 actions bump — DONE 2026-05-30.** Swept `.github/workflows/*` to node24-runtime majors: `actions/checkout@v6`, `actions/setup-node@v6`, `actions/upload-artifact@v7`, `actions/cache@v5`, `docker/setup-buildx-action@v4`, `docker/login-action@v4`, `docker/build-push-action@v7`, `pnpm/action-setup@v6`. **Intentionally NOT bumped:** `docker/metadata-action@<SHA>` (security SHA pin), `aquasecurity/trivy-action`/`rtCamp/action-slack-notify` (Docker container actions), `peter-evans/create-pull-request@v6` (bump when touching `generate-openapi.yml`). `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` is the fallback opt-in. **Follow-up 2026-05-31:** **`pnpm/action-setup@v6` fails when it runs AFTER `actions/setup-node@v6`** — install pnpm BEFORE Node (the order `typecheck.yml` uses). Regressed then fixed `e2e.yml`. Also **deleted both upstream-leftover container workflows** (`build-deploy-develop-container.yaml`, `build-deploy-container.yml` — pushed to `bigcapitalhq/*` Docker Hub, no creds, red X; superseded by `deploy.yml`). Remaining: `deploy.yml`, `e2e.yml`, `typecheck.yml`, `generate-openapi.yml`.
- **Containerd layer-extraction flake on rapid back-to-back deploys (2026-05-21).** `failed to extract layer ... no such file or directory` in overlayfs; VPS-side `docker compose pull` retry succeeds. **Monthly housekeeping cron WIRED 2026-05-31** (`deploy/vps-docker-prune.sh` + `deploy/bigcapital-{sandbox,uat}-docker-prune.cron`, 04:30/04:50 UTC on the 1st). Deliberately **scoped** (not `system prune -af`): only the env's own exited containers (by `container_name` prefix) + bigcapital images >14d not backing a container. `vps-deploy.sh:80` does a global `image prune -af` per deploy — confirmed fine (VPS daemon effectively dedicated to bigcapital, no co-tenant blast radius). Re-install commands in `docs/CI-CD.md`.
