...
### OPEN (unresolved 2026-05-30) — edit-invoice save returns 400

Saving an edited sale invoice (`PUT /api/sale-invoices/8`) returns 400 with only a silent `AxiosError` in the console — no toast. Investigation so far: the global `ClassValidation.pipe.ts` is `whitelist: true` but NOT `forbidNonWhitelisted`, so extra payload props are stripped (not the cause); a field-level DTO validation failure is, and its detail is in `response.data.message` (array), which the form's `onError` (reads `response.data.errors`) swallows. The session's recent invoice changes are ruled out: empty `discount` → `null` (skipped by the custom `@IsOptional`), and `discount_type: 'amount'` is a valid enum. Invoice 8 in the (stale) audit snapshot looks valid — 3 sellable service items, no discount/categories, fully paid $572/$572. **Next step**: capture the actual `message` array from the Network response (or what the user edited) to name the failing field; then consider surfacing `response.data.message` as a toast so these are no longer invisible. Note: invoice 8 is fully paid, so an edit lowering the total below $572 would trip `INVOICE_AMOUNT_SMALLER_THAN_PAYMENT_AMOUNT` (that path _should_ toast, so less likely the silent-400 cause).
