...
3. **Best-effort graceful degrade (+ a docker `env_file` config trap).** With the URL and tenant fixed, `/latest` returned a clean **400 `EX_RATE_SERVICE_API_KEY_REQUIRED`** — and the env check revealed why: `OPEN_EXCHANGE_RATE_APP_ID` on staging was literally `# [CHANGE] if using exchange rates`. The **inline comment from the `.env.production` template leaked in as the value** — docker's `env_file` parser (this host) does NOT strip a trailing `# …` comment; everything after `=` becomes the value. So the app id was non-blank garbage → my first-pass degrade (which only short-circuited on a _blank_ key) was skipped → the provider was called and rejected it. Fix: make auto-rate **best-effort** — skip the call when the key is blank, otherwise attempt it but **catch ANY provider failure** (rejected/garbage key, base-currency not allowed on plan, network) and fall back to `exchangeRate: 1`, logging a `warn`. Now the form always falls back to manual entry without a console error, regardless of env state. Operational follow-up: clean staging's `.env` (`OPEN_EXCHANGE_RATE_APP_ID=` with no inline comment, or a real paid key) and audit other value lines for the same comment leak. Auth/DB work, so the critical secrets were filled in properly during setup; this was the one untouched template line.

Net: auto-rate population is not enabled on staging (no valid provider key, by design), but the document forms no longer throw or log — manual rate entry works, which is all the FX UAT (§2) needs.
