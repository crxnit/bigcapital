...
Upstream `bigcapitalhq/bigcapital` ships its own CI workflows that push to Docker Hub `bigcapitalhq/server` and `bigcapitalhq/webapp`. We don't consume those — the leftover `.github/workflows/build-deploy-container.yml` and `build-deploy-develop-container.yaml` were **deleted** in the 2026-05-30 Node-24 sweep (see "Open items & bring-up status"). Remaining workflows: `deploy.yml`, `e2e.yml`, `typecheck.yml`, `generate-openapi.yml`.

The fork's deploy path is entirely the new `deploy.yml`. On future upstream merges, re-deleting any reintroduced `build-deploy-*container*` workflow is safe — we don't use their output.

## Open items & bring-up status

Migrated from `.claude/CLAUDE.md` (which now carries only the still-PENDING one-liners + a pointer here). Status as of 2026-05-31.
