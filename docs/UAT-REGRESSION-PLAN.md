...
| BUG-003        | P2  | §2 FX (pre-existing) | **Fixed, pending deploy** (`exchangeRates.tsx:26`)                 | Latest-exchange-rate auto-fetch 404s: webapp called `/api/exchange_rates/latest` (underscore); server route is `/api/exchange-rates/latest` (hyphen). **Upstream bug since 2024-01-28**, not a changeset regression. Non-blocking — enter the rate manually. FX rate won't auto-populate until deployed. |
