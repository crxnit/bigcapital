...
| DATA-008       | P3  | §4 staging data (not code) | **Open (data hygiene, provenance unknown)**                               | Mercury **Savings (1621)** and **Checking (1625)** share **35 identical** transactions (same date/amount/payee) — a whole statement loaded into two accounts (someone else entered it; method unknown). Double-counts cashflow if both copies are categorized. **Not a changeset regression** — nothing under test creates cross-account dupes from one import; pre-existing data. Cleanup: delete one account's copies before trusting cashflow reports. Made §4.5/§4.7 untrustworthy on this org → those not exercised.                                                                                                                                           |
