...
| REGRESSION-006 | P1  | §3 allocations (edit) | **Fixed** (`EditCreditNote.service.ts:124`)                           | Editing a **published credit note** updated the doc but left its **GL stale** — reproduced: CN1 £15→£20, child+header=20 but GL still 13.35 (£15×0.89). Internally balanced so §1 gate misses it. Root cause: emitted bare `upsertGraph` result (lacks `openedAt` for already-published notes) → `onEdited` GL subscriber's `isPublished` gate falsy → rewrite skipped. **Affects all published-CN edits, not just allocations.** Fix: `upsertGraph` → `upsertGraphAndFetch` (mirrors EditSaleInvoice). Post-mortem + CLAUDE.md gotcha added. **Data cleanup:** re-save credit notes edited pre-fix (CN1 → 17.80).                                                  |
