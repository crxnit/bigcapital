...
| DATA-008       | P3  | §4 staging data (not code) | **Resolved 2026-06-02** (data cleanup)                                    | **ALL 26** Mercury **Savings (1621)** transactions duplicated Mercury **Checking (1625)** — pre-existing data someone else entered; method unknown. **Not a changeset regression.** **Cleanup (run on staging VPS):** deleted account 1621's `UNCATEGORIZED_CASHFLOW_TRANSACTIONS` rows + cleared their `MATCHED_BANK_TRANSACTIONS`/`RECOGNIZED_BANK_TRANSACTIONS` FK children in one trx → `remaining 0`. **Verified:** (a) account 1621 has **zero `ACCOUNTS_TRANSACTIONS` legs** — the dupes never posted GL (the "2 categorized" note was inaccurate/already-handled, so no orphaned legs to reverse); (b) global GL gate empty (no collateral imbalance). **Unblocks §4.5 / §4.7.**                                                                                |
