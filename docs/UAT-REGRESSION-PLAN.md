...
| §4.7 edit categorization GL rewrite    | Backend ✓ (`EditCategorizeBankTransaction.service.ts:113` patch→revert→write in one UoW trx; re-reads patched row). **UI ✗→fixed**: dialog crashed the app (**BUG-012**, Formik field outside Formik); fixed local, **needs deploy** before live GL re-test.                                                          |
