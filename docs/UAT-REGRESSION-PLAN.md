...
| DATA-008       | P3  | §4 staging data (not code) | **Open (data hygiene, provenance unknown)**                               | **ALL 26** Mercury **Savings (1621)** transactions duplicate Mercury **Checking (1625)** — the entire Savings account mirrors Checking (someone else entered it; method unknown). Double-counts cashflow; 2 Savings rows already categorized (posted GL). **Not a changeset regression** — nothing under test creates cross-account dupes from one import; pre-existing data. Cleanup: uncategorize the 2 posted rows, then delete account 1621's txns (or the phantom account). Made §4.5/§4.7 untrustworthy on this org → those not exercised.                                                                                                                    |
