...
| §4.7 edit categorization GL rewrite    | ✗✗ **TWO bugs found live + fixed (pending deploy)**: **BUG-012** dialog crashed the app (Formik field outside Formik); **BUG-013** (P0) editing silently **destroyed the GL** (blank type → zero legs, §1-gate-invisible). Both fixed local. CF-94 data needs recovery. **Backend GL revert→rewrite logic itself is sound** — the bug was a blanked type fed into it. Needs deploy + clean re-test. |
| §4.8 account-delete cascade            | ✓ `DeleteUncategorizedTransactionsOnAccountDeleting.ts` clears matched+recognized FK children before uncategorized; empty-list guard _(live pending)_                                                                                                                                                                                                                                               |
...
| §5.1 CC as Bill payment account        | ✓ `BillPaymentValidators.service.ts:74` allows CREDIT*CARD; `BillPaymentGL.ts` DR AP / CR payment acct, both `localAmount` *(live pending)\_                                                                                                                                                                                                                                                        |
| §5.2 CC in Quick Payment Made          | ✓ `QuickPaymentMadeFormFields.tsx:144` includes CREDIT_CARD in `filterByTypes`                                                                                                                                                                                                                                                                                                                      |
| §5.3 vendor-credit apply-to-bills      | ✓ `VendorCreditApplyBillsSyncSubscriber` `@OnEvent` created+deleted, registered _(live pending)_                                                                                                                                                                                                                                                                                                    |
| §5.4 vendor-credit refund sync         | ✓ `RefundSyncVendorCreditBalanceSubscriber` wired (not commented-out), create+delete _(live pending)_                                                                                                                                                                                                                                                                                               |
| §5.5 net-zero payment ε                | ✓ `> 0.005` in `PaymentMadeForm.tsx:105` + `PaymentReceiveForm.tsx:114`                                                                                                                                                                                                                                                                                                                             |
| §5.6 Payment Received FX               | ✓ `PaymentReceivedGL` DR deposit / CR AR both `localAmount`, FX split → cash↔AR only _(live pending)_                                                                                                                                                                                                                                                                                              |
